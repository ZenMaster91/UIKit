...
public class AKS extends Thread {
	...
	public boolean isPrime() {
		...
		if (verbose)
			System.out.println(n
					+ " is not a perfect power of any integer less than its square root");
		...
		for (BigInteger i = BigInteger.valueOf(2); i.compareTo(r) <= 0; i = i
				.add(BigInteger.ONE)) {
		...
		// X^r - 1
		Poly modPoly = new Poly(BigInteger.ONE, r.intValue())
				.minus(new Poly(BigInteger.ONE, 0));
		// X^n (mod X^r - 1, n)
		...
		for (int i = 1; i <= limit; i++) {
			...
			Poly p = new Poly(BigInteger.ONE, 1).plus(polyI).modPow(n, modPoly,
					n);
			if (!outcome.equals(p)) {
				if (verbose)
					System.out.println("(x+" + i + ")^" + n + " (mod x^" + r
							+ " - 1, " + n + ") = " + outcome);
				if (verbose)
					System.out.println("x^" + n + " + " + i + " (mod x^" + r
							+ " - 1, " + n + ") = " + p);
	...
	BigInteger totient(BigInteger n) {
		...
		for (BigInteger i = BigInteger.valueOf(2); n
				.compareTo(i.multiply(i)) > 0; i = i.add(BigInteger.ONE)) {
