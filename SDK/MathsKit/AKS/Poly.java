package MathsKit.AKS;

import java.math.BigInteger;
import java.util.Arrays;

/**
 * This is a polynomial class that uses BigIntegers as the coefficients It
 * supports mod operations where the modulus is an either or another polynomial
 * 
...
public class Poly {
	...
	public Poly plus(Poly p) {
		...
		for (int i = 0; i <= maxDegree; i++)
			sum.monos[i] = (i <= this.degree && i <= p.degree)
					? p.monos[i].add(this.monos[i])
	...
	public Poly minus(Poly p) {
		...
		for (int i = 0; i <= maxDegree; i++)
			difference.monos[i] = (i <= this.degree && i <= p.degree)
					? this.monos[i].subtract(p.monos[i])
	...
	static void updateDegree(Poly p) {
		...
		while (p.monos[p.degree].compareTo(BigInteger.ZERO) == 0
				&& p.degree > 0)
			p.degree--;
	}
	...
	public Poly times(Poly p) {
		...
		for (int i = 0; i <= this.degree; i++) {
			for (int j = 0; j <= p.degree; j++)
				product.monos[i + j] = product.monos[i + j]
						.add(this.monos[i].multiply(p.monos[j]));
		}
	...
	public Poly mod(Poly m) {
		...
		while (remainder.degree >= m.degree) {
			...
			Poly sub = new Poly(m).times(
					new Poly(remainder.coefficient(remainder.degree), diff));
	 ...
	 */
	public Poly modPow(BigInteger exponent, Poly mPoly,
